...
\frame{
  ...
  \item City University, Londres
  \item CCARH, Stanford University, San Francisco
  \item CCRMA, Stanford University, San Francisco (no fim do mês)
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
  \item Identificação de acordes
  \item Análise harmônica funcional com romanos
  \item Identificação automática de notas melódicas
  \item Funcionalidade musicológica básica
    \begin{itemize}
    \item cadências
    \item cruzamento de vozes
    \end{itemize}
  \item Entrada e saída em Lilypond
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
  \item Segmentação
  \item Encontrar a tonalidade (regiões)
  \end{itemize}
}
\frame{
  \frametitle{Segmentação (fácil)}
  \framesubtitle{segmentando a peça em acordes}
  \includegraphics[scale=.8]{segments-easy}
}

\frame{
  \frametitle{Segmentação (nada fácil)}
  \framesubtitle{segmentando a peça em acordes}
  \includegraphics[scale=.4]{segments-not-easy}
}

\frame{
  \frametitle{Saída do Rameau (identificação de acordes)}
  \hspace{-3em}
  \includegraphics[scale=.45]{chord}
}

\frame{
  \frametitle{Saída do Rameau (análise funcional com romanos)}
  \hspace{-6em}
  \includegraphics[scale=.6]{roman}
}

\frame{
  \frametitle{Média de acuracidade (10 corais)}
  \begin{center}
    \begin{tabular}{rrrr}
...
\frame{
  \frametitle{Musicologia computacional}

  Identificar elementos musicais em um grande corpus musical em pouco
  tempo.

  Exemplos de perguntas:

  \begin{itemize}
  \item Quais as cadências mais comuns na obra de Beethoven?
  \item Quais as diferenças de âmbito orquestral entre todas as obras
    de Beethoven, Wagner e R. Strauss
  \end{itemize}
}

\frame{
  \frametitle{Respondendo algumas perguntas com o Rameau}

  Nos 366 corais de Bach,

  \begin{itemize}
  \item qual o âmbito geral utilizado?
  \item há cruzamentos de vozes? entre quais vozes?
  \item há acordes de sexta aumentada?
  \item há quintas e oitavas consecutivas?
  \item como as sétimas são resolvidas? ascendentemente,
    descendentemente?
  \item quais as cadências finais mais utilizadas?
  \end{itemize}
}

\frame{
  \frametitle{Rameau new generation}
  Toolkit para análise musical a partir do Humdrum
}
\subsection{Goiaba}
...
\includegraphics[scale=.6]{esquema-tex}
}
